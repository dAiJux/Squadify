...
public class MatchmakingService {
    ...
    public Flux<ProfileResponse> getCandidates(String currentUserId, int limit, String afterId) {
        return profileRepository.findByUserId(currentUserId)
                .flatMapMany(myProfile -> {
                    ...
                    Mono<List<String>> matchedIdsMono = matchRepository
                            .findByUser1IdOrUser2Id(currentUserId, currentUserId)
                            .flatMap(match -> {
                    ...
                    return Mono.zip(swipedIdsMono, matchedIdsMono)
                            .flatMapMany(tuple -> {
                                ...
                                if (excludeIds.size() <= EXCLUDE_NIN_THRESHOLD) {
                                    ...
                                    baseQuery.fields().include("_id").include("userId").include("games")
                                            .include("schedules").include("playStyle");
                                    ...
                                    fallbackQuery.fields().include("_id").include("userId").include("games")
                                            .include("schedules").include("playStyle");
    ...
    private Mono<SwipeResponse> checkAndCreateMatch(String swiperId, String targetId) {
        return swipeRepository.findBySwiperIdAndTargetIdAndType(targetId, swiperId, SwipeType.LIKE)
                .flatMap(existingLike -> {
                    ...
                    String matchId = user1Id + "-" + user2Id;

                    return matchRepository.save(newMatch)
                            .map(m -> new SwipeResponse(true, matchId))
                            .onErrorResume(e -> Mono.just(new SwipeResponse(false, null)));
                })
                .defaultIfEmpty(new SwipeResponse(false, null));
    }
}